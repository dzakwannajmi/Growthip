...
- [Verifier V4 Test Coverage Gap](#verifier-v4-test-coverage-gap--found-and-fixed)
...
## Verifier V4 Test Coverage Gap — Found and Fixed

**Severity: None in production (fail-closed if ever exploited; found before
any production impact was possible).**
**Status: Fixed. Test coverage added.**

### What was found

The V4 verifier (`growthip-merkle-verifier-v4`) shipped with zero tests
that proved it actually accepted a valid, real V4 proof. The only accept-path
test that existed was `#[ignore]`d (it used a V3 depth-3 proof artifact,
invalid against the depth-20 V4 verifying key), and the remaining "V4"
tests either asserted nothing or rejected a mismatched-circuit proof for
the wrong reason. In practice, no test in the workspace proved the V4
verifier could ever return `true`.

This gap was discovered while auditing the deposit-amount-aware claims
fix for V4 readiness. Writing the missing accept-path test surfaced a
real bug — but not where it mattered.

### The bug that was actually found

A standalone Node.js script used only for generating local test
fixtures, `scripts/convert_growthip_merkle_note_v4_snarkjs.js`, encoded
G2 (Fp2) coordinates in the wrong order — `[c0, c1]` instead of the
`[c1, c0]` ("imaginary, real") order Soroban's `Bn254G2Affine::from_bytes`
expects. Running a proof generated from this script's output against the
real, deployed verifier failed with `Error(Crypto, InvalidInput)` /
`"bn254 G2: point not on curve"` — a **fail-closed** rejection, not an
accepted-forged-proof, but it meant no proof (valid or otherwise) run
through this script could ever verify.

### Why production was never affected

Two independent code paths handle G2 encoding, and **neither of them is
this script**:

1. **`contracts/growthip-merkle-verifier-v4/build.rs`** — the actual
   code that embeds the verification key into the deployed WASM binary.
   Its `serialize_g2_point()` correctly emits `x.c1` (imaginary) before
   `x.c0` (real).
2. **`apps/web/src/lib/zkp.ts`** — the actual code every real user's
   browser runs to serialize a proof before calling `claim_to()`. Its
   `g2Hex()` helper has the same correct `[c1, c0]` order, with an
   explicit doc comment stating the convention.

Both were checked line-by-line and independently confirmed correct
*before* the buggy script was identified as the outlier, not the norm.
The already-deployed V4 verifier contract's embedded verification key
was generated by `build.rs`, not the broken script, and was never
regenerated with the wrong ordering — so no redeploy was required.

### The fix

`scripts/convert_growthip_merkle_note_v4_snarkjs.js`'s `g2ToHex()` was
corrected to match the same `[c1, c0]` order already proven correct in
`build.rs` and `zkp.ts` (and previously proven live on testnet by V3.1).
Three tests were added to `growthip-merkle-verifier-v4`'s test suite
using a freshly regenerated, real V4 proof:

* `v4_valid_proof_accepted` — a real V4 proof must verify. This is the
  test that would have caught a G2 ordering bug in production code, had
  one existed; its pass confirms `build.rs` and `zkp.ts`'s encoding is
  correct, not merely assumed correct.
* `v4_tampered_public_inputs_rejected` — starting from the valid proof,
  flipping one bit in any public input must be rejected.
* `v4_tampered_proof_rejected` — flipping one bit anywhere in the proof
  bytes (A, B, or C) must be rejected.

All three pass against the currently deployed V4 verifier's logic.

### Lesson

A test that only confirms rejection (of an already-broken or
mismatched-circuit proof) proves nothing about whether the accept path
works. The gap here wasn't a logic bug in shipped code — it was an
absence of the one test that would have caught a logic bug, had one
existed, in either shipped code path. Both shipped paths turned out
correct; that is now demonstrated, not assumed.
