...
const vk = JSON.parse(
  fs.readFileSync(
    path.join(buildDir, "growthip_merkle_note_v3_verification_key.json"),
    "utf8"
  )
);
...
const proof = JSON.parse(
  fs.readFileSync(
    path.join(buildDir, "growthip_merkle_note_v3_proof.json"),
    "utf8"
  )
);
...
const publicInputs = JSON.parse(
  fs.readFileSync(
    path.join(buildDir, "growthip_merkle_note_v3_public.json"),
    "utf8"
  )
);
...
// V3 must have exactly 3 public inputs: root, nullifierHash, recipientHash
if (publicInputs.length !== 3) {
  throw new Error(
    `Expected 3 public inputs for V3, got ${publicInputs.length}`
  );
}

// G1/G2 helpers — same conversion as V2
// G2: snarkjs stores [real, imaginary]; Soroban verifier expects [imaginary, real]
function g1(point) {
...
const parameters = {
  ...
  metadata: {
    circuit:        "growthip_merkle_note_v3.circom",
    public_outputs: ["root", "nullifierHash", "recipientHash"],
    private_inputs: ["secret", "nullifier", "pathElements", "pathIndices", "recipientHash"],
    note: "V3: recipientHash is now private and cryptographically bound inside commitment. " +
          "G2 points: snarkjs [real, imag] → Soroban [imag, real].",
  },
};

fs.writeFileSync(
  path.join(buildDir, "growthip_merkle_note_v3_parameters.json"),
...
if (publicInputsHex.length !== 192) {
  throw new Error(
    `Expected public inputs hex length 192 (3×64), got ${publicInputsHex.length}`
  );
}

fs.writeFileSync(
  path.join(buildDir, "growthip_merkle_note_v3_proof_abc.hex"),
  proofHex
);
fs.writeFileSync(
  path.join(buildDir, "growthip_merkle_note_v3_public_inputs.hex"),
  publicInputsHex
);
fs.writeFileSync(
  path.join(buildDir, "growthip_merkle_note_v3_public_inputs.json"),
  JSON.stringify(publicInputs, null, 2)
);

console.log("=== Converted Growthip Merkle note V3 artifacts ===");
...
console.log("  circuits/build/growthip_merkle_note_v3_parameters.json");
console.log("  circuits/build/growthip_merkle_note_v3_proof_abc.hex");
console.log("  circuits/build/growthip_merkle_note_v3_public_inputs.hex");
console.log("  circuits/build/growthip_merkle_note_v3_public_inputs.json");