...
#[contract]
pub struct GrowthipMerkleVerifierV31;

#[contractimpl]
impl GrowthipMerkleVerifierV31 {
...
mod test {
    ...
    fn public_inputs_from_hex(env: &Env, hex_str: &str) -> Vec<BytesN<32>> {
        ...
        assert!(
            clean.len() % 64 == 0,
            "public input hex must be multiple of 64"
        );
    ...
    fn test_verify_growthip_merkle_note_v3_proof() {
        ...
        let contract_id = env.register(GrowthipMerkleVerifierV31, ());
        let client = GrowthipMerkleVerifierV31Client::new(&env, &contract_id);

        let proof_hex =
            include_str!("../../../circuits/build/growthip_merkle_note_v3_1_proof_abc.hex");
        let public_inputs_hex =
            include_str!("../../../circuits/build/growthip_merkle_note_v3_1_public_inputs.hex");
