...
#[contract]
pub struct GrowthipNoteVerifier;

#[contractimpl]
impl GrowthipNoteVerifier {
...
mod test {
    ...
    #[test]
    fn test_verify_growthip_note_v0_proof() {
        let env = Env::default();

        let contract_id = env.register(GrowthipNoteVerifier, ());
        let client = GrowthipNoteVerifierClient::new(&env, &contract_id);

        let proof_hex = include_str!("../../../circuits/build/growthip_note_proof_abc.hex");
        let public_inputs_hex =
            include_str!("../../../circuits/build/growthip_note_public_inputs.hex");
