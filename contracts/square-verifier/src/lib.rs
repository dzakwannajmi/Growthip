...
mod test {
    ...
    #[test]
    fn test_verify_square_proof() {
        let env = Env::default();

        let contract_id = env.register(SquareVerifier, ());
        let client = SquareVerifierClient::new(&env, &contract_id);

        let proof_hex = include_str!("../../../circuits/build/proof_abc.hex");
        let proof_bytes = bytes_from_hex(&env, proof_hex);

        let public_inputs = vec![&env, fr_4(&env)];
