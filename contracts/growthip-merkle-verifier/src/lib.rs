...
mod test {
    ...
    #[test]
    fn test_verify_growthip_merkle_note_v1_proof() {
        let env = Env::default();

        let contract_id = env.register(GrowthipMerkleVerifier, ());
        let client = GrowthipMerkleVerifierClient::new(&env, &contract_id);

        let proof_hex = include_str!("../../../circuits/build/growthip_merkle_note_proof_abc.hex");
        let public_inputs_hex =
            include_str!("../../../circuits/build/growthip_merkle_note_public_inputs.hex");
