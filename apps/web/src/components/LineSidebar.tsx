...
import { useRef, useState, useCallback, useEffect, type CSSProperties, type PointerEvent as ReactPointerEvent } from "react";
...
const DEFAULT_ITEMS = [
  "Overview", "Components", "Animations", "Backgrounds", "Showcase",
  "Playground", "Templates", "Changelog", "Community", "Resources",
...
export default function LineSidebar({
  ...
  useEffect(
    () => () => {
      // Reset rafRef to null (not just cancel) -- otherwise React
      // StrictMode's dev-only mount→cleanup→remount cycle leaves this
      // ref holding a stale, already-cancelled animation frame id.
      // startLoop()'s `if (rafRef.current != null) return;` guard would
      // then see that stale non-null value and conclude the loop is
      // "already running", silently skipping requestAnimationFrame on
      // the real remount -- the effect nothing ever visibly updates
      // until a hard reload creates a genuinely fresh ref.
