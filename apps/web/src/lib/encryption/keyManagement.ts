...
export interface CreateIdentityResult {
  ...
  /** 12-word recovery phrase. MUST be shown to the user exactly once and
   * never logged, stored, or transmitted by this app. The caller (UI
   * layer) is responsible for the "write this down" confirmation flow
   * before treating setup as complete. */
  recoveryPhrase: string;
}

/**
 * Creates a brand-new encryption identity: generates an X25519 keypair,
 * generates a recovery phrase, wraps the private key under both the
 * given password and the recovery phrase independently (an "OR gate" --
 * either secret alone is sufficient to unlock later), persists the
 * wrapped record, and leaves the session unlocked (non-extractable
 * private key held in memory) afterward.
 *
 * Does NOT submit the public key on-chain -- that's a separate concern
 * (a signed Soroban transaction), left to the caller after this resolves.
 */
export async function createIdentity(password: string): Promise<CreateIdentityResult> {
  if (await hasIdentity()) {
    throw new Error(
      "An encryption identity already exists in this browser. Use rotateIdentity() to replace it, or unlock the existing one instead.",
    );
  }

  const keyPair = await generateX25519KeyPair();
  const publicKeyRaw = await exportPublicKeyRaw(keyPair.publicKey);
  const privateKeyBytes = await exportPrivateKeyPkcs8(keyPair.privateKey);

  const recoveryPhrase = bip39.generateMnemonic(wordlist, RECOVERY_PHRASE_STRENGTH_BITS);
  ...
  const [passwordHash, recoveryHash] = await Promise.all([
    runArgon2idInWorker(password, passwordSalt, KDF_PARAMS),
    runArgon2idInWorker(recoveryPhrase, recoverySalt, KDF_PARAMS),
  ]);

  const passwordWrappingKey = await importWrappingKey(passwordHash);
  const recoveryWrappingKey = await importWrappingKey(recoveryHash);

  const wrappedByPassword = await wrapKeyBytes(passwordWrappingKey, privateKeyBytes);
  const wrappedByRecovery = await wrapKeyBytes(recoveryWrappingKey, privateKeyBytes);

  const identity: StoredIdentity = {
    id: "primary",
    publicKeyRaw,
    wrappedByPassword: wrappedByPassword.ciphertext,
    passwordWrapIv: wrappedByPassword.iv,
    passwordKdfSalt: passwordSalt,
    passwordKdfParams: KDF_PARAMS,
    wrappedByRecovery: wrappedByRecovery.ciphertext,
    recoveryWrapIv: wrappedByRecovery.iv,
    recoveryKdfSalt: recoverySalt,
    recoveryKdfParams: KDF_PARAMS,
    createdAt: Date.now(),
  };

  await saveIdentity(identity);

  // Leave the session unlocked immediately after setup -- re-importing
  // non-extractable directly from the freshly-generated keyPair avoids
  // an unnecessary extra unwrap round-trip.
  sessionPrivateKey = await importPrivateKeyNonExtractable(privateKeyBytes);
  resetAutoLockTimer();

  return { publicKeyRaw, recoveryPhrase };
}
...
export async function unlockWithRecoveryPhrase(recoveryPhrase: string): Promise<void> {
  const identity = await loadIdentity();
  if (!identity) throw new Error("No encryption identity found in this browser.");

  const normalized = recoveryPhrase.trim().toLowerCase();
  if (!bip39.validateMnemonic(normalized, wordlist)) {
    throw new Error("This does not look like a valid recovery phrase.");
  }

  await unlockWithSecret(
    normalized,
    identity.wrappedByRecovery,
    identity.recoveryWrapIv,
    identity.recoveryKdfSalt,
    identity.recoveryKdfParams,
  );
}
...
/**
 * Serializes the current stored identity into a downloadable JSON backup.
 * Safe to store anywhere (cloud drive, USB, email to yourself) -- every
 * sensitive field is already AES-GCM-wrapped; this file alone does not
 * grant access without the password or recovery phrase.
 */
export async function exportBackupFile(): Promise<Blob> {
  const identity = await loadIdentity();
  if (!identity) throw new Error("No encryption identity to back up.");
  ...
  const backup: BackupFileV1 = {
    ...
    exportedAt: Date.now(),
    identity: {
      publicKeyRaw: bytesToBase64(identity.publicKeyRaw),
      wrappedByPassword: bytesToBase64(identity.wrappedByPassword),
      passwordWrapIv: bytesToBase64(identity.passwordWrapIv),
      passwordKdfSalt: bytesToBase64(identity.passwordKdfSalt),
      passwordKdfParams: identity.passwordKdfParams,
      wrappedByRecovery: bytesToBase64(identity.wrappedByRecovery),
      recoveryWrapIv: bytesToBase64(identity.recoveryWrapIv),
      recoveryKdfSalt: bytesToBase64(identity.recoveryKdfSalt),
      recoveryKdfParams: identity.recoveryKdfParams,
...
/**
 * Restores an identity from a previously-exported backup file (e.g. on a
 * new device/browser). Does NOT automatically unlock the session --
 * callers must still call unlockWithPassword()/unlockWithRecoveryPhrase()
 * afterward with whichever secret the user has.
 */
export async function importBackupFile(file: Blob): Promise<void> {
  ...
  if (parsed.format !== "growthip-encryption-backup" || parsed.version !== 1) {
    throw new Error("This file is not a recognized Growthip encryption backup.");
  }

  const identity: StoredIdentity = {
    id: "primary",
    publicKeyRaw: base64ToBytes(parsed.identity.publicKeyRaw),
    wrappedByPassword: base64ToBytes(parsed.identity.wrappedByPassword),
    passwordWrapIv: base64ToBytes(parsed.identity.passwordWrapIv),
    passwordKdfSalt: base64ToBytes(parsed.identity.passwordKdfSalt),
    passwordKdfParams: parsed.identity.passwordKdfParams,
    wrappedByRecovery: base64ToBytes(parsed.identity.wrappedByRecovery),
    recoveryWrapIv: base64ToBytes(parsed.identity.recoveryWrapIv),
    recoveryKdfSalt: base64ToBytes(parsed.identity.recoveryKdfSalt),
    recoveryKdfParams: parsed.identity.recoveryKdfParams,
    createdAt: parsed.identity.createdAt,
  };
...
/**
 * Returns the public key of the currently stored identity (if any),
 * without requiring an unlocked session -- public keys are never secret.
