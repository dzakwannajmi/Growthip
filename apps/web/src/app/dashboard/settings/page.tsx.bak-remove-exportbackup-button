...
export default function SettingsPage() {
  ...
  return (
    <div className="p-4 md:p-8 lg:p-10 w-full min-h-full bg-[#FAFAFA] dark:bg-[#0A0A0A]">
      <div className="max-w-[700px] mx-auto pb-20 flex flex-col gap-6">
        ...
        {!address ? (
          <div className="rounded-2xl p-6 text-center border border-[#E5E5E5] dark:border-[#2A2A2A] bg-white dark:bg-[#1A1A1A]">
            ...
            <div>
              ...
              <div className="rounded-2xl overflow-hidden bg-white dark:bg-[#1A1A1A] border border-[#E5E5E5] dark:border-[#2A2A2A]">
                ...
                <button onClick={() => setShowGrIdentity((p) => !p)} className="w-full p-4 flex items-center justify-between transition-colors hover:bg-[#F5F5F5] dark:hover:bg-[#2A2A2A]">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center dark:bg-[#2A2A2A]" style={{ background: "#F5F5F5" }}>
                      <Icon icon="ph:eye-slash-bold" className="text-xl text-[#0A0A0A] dark:text-[#F0F0F0]" />
                    </div>
                    <div className="text-left">
                      <div className="font-bold text-sm text-[#0A0A0A] dark:text-[#FAFAFA]">gr Identity</div>
                      <div className="text-xs text-[#737373] dark:text-[#8A8A8A]">Fully private V5 tipping — separate from Security &amp; Private Notes</div>
                    </div>
                  </div>
                  <Icon icon={showGrIdentity ? "ph:caret-up-bold" : "ph:caret-right-bold"} className="text-[#A3A3A3] dark:text-[#6A6A6A]" />
                </button>
                <div className="dark:bg-[#2A2A2A]" style={{ height: "1px", background: "#E5E5E5" }} />
            ...
            <div>
              ...
              <div className="rounded-2xl bg-white dark:bg-[#1A1A1A] border border-[#E5E5E5] dark:border-[#2A2A2A] overflow-hidden">
                ...
                <div className="border-b border-[#E5E5E5] dark:border-[#2A2A2A]" style={{ padding: "14px 16px", display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <div>
                    <p className="text-[#0A0A0A] dark:text-[#F5F5F5]" style={{ fontSize: "13px", fontWeight: 700, margin: "0 0 1px" }}>Available Tokens</p>
                    <p className="text-[#A3A3A3] dark:text-[#6A6A6A]" style={{ fontSize: "11px", margin: 0 }}>Stellar tokens you can receive as tips</p>
                  </div>
                </div>
                {/* Token list */}
                {[
                  ...
                  <div key={token.name} className={i < arr.length - 1 ? "border-b border-[#F5F5F5] dark:border-[#232323]" : ""} style={{ padding: "14px 16px", opacity: token.available ? 1 : 0.55, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                      ...
                      <div>
                        ...
                        <p className="text-[#A3A3A3] dark:text-[#6A6A6A]" style={{ fontSize: "11px", margin: 0 }}>{token.fullName}</p>
                      </div>
                    </div>
                    {token.available
                      ? <div style={{ textAlign: "right" }}>
                          <p className="text-[#737373] dark:text-[#8A8A8A]" style={{ fontSize: "12px", margin: "0 0 2px" }}>Fee: {token.fee}</p>
                          <p className="text-[#A3A3A3] dark:text-[#6A6A6A]" style={{ fontSize: "11px", margin: 0 }}>{token.decimals} decimals</p>
                        </div>
                      : <span className="bg-[#F5F5F5] dark:bg-[#2A2A2A] text-[#A3A3A3] dark:text-[#6A6A6A]" style={{ fontSize: "11px", fontWeight: 700, padding: "3px 10px", borderRadius: "999px" }}>Coming soon</span>
                    }
                  </div>
                ))}
              </div>
